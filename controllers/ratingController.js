...
module.exports = {
        ...
        try {
            ...
            { $group: { _id: '$product' }, averateRating: { $avg: '$rating' } }
                ...
                if (restaurants.length > 0) {
                    const averateRating = restaurants[0].averageRating;
                ...
                if (foods.length > 0) {
                    const averateRating = foods[0].averageRating;
    ...
    checkUserRating : async (req,res) => {
        ...
        try {
            ...
            if(existingRating){
                res.status(200).json({status : true , message : "You have already rated this restuarant."});
            }else{
                res.status(200).json({status : false , message : "User has not rate this restuarant."});
