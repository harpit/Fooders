        ...
        try {
            ...
            if (cartItem) {
                ...
                if (cartItem.quantity > 1) {
                    ...
                    await cartItem.save();
                    res.status(200).json({ status: true, message: "Product Quantity successfully decreased" })
                } else {
                    await Cart.findOneAndDelete({ _id: id });
                    res.status(200).json({ status: true, message: "Product successfully remove form cart" })
                }
            } else {
                res.status(200).json({ status: true, message: "Cart Items not found" })
            }
        } catch (error) {
            res.status(500).json({ status: false, message: error.message });
        }
