...
module.exports = {
    addFood: async (req, res) => {
        ...
        try {
            ...
            await newFood.save();
            res.status(201).json({ status: true, message: "Food Added Sucessfully" });
        } catch (error) {
  ...
  searchFoods: async (req, res) => {
    ...
    getRandomFoodsByCategoryAndCode: async (req, res) => { // Rename function
        const {category , code } = req.params;
        try {
            let foods;
            ...
            if(!foods || foods.length === 0){
                foods = await Food.aggregate([ // Fix assignment
