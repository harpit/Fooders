...
const verifyVendor = (req, res, next) => {
    verifyToken(req, res, () => {
        ...
        if (req.user.userType === "Admin", req.user.userType === "Vendor") {
            ...
            return res.status(401).json({ status: false, message: "You are not allowed to access the routes(vendor)." });
        }
    })
};
...
const verifyDriver = (req, res, next) => {
    verifyToken(req, res, () => {
        ...
        if (req.user.userType === "Driver") {
            ...
            return res.status(401).json({ status: false, message: "You are not allowed to access the routes(Driver)." });
        }
    })
};
